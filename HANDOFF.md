...
### 10.1 検証結果を採用してよいかの判定 (2026-08-13 制定・恒久)

過去データで「Aの方がBより良い」と出たとき、それを実際の設定変更として採用してよいか
どうかの家のルール。**4つ全部を満たしたときだけ採用する。1つでも欠けたら据え置き。**

1. **前半(〜2014)と後半(2015〜)の両方で同じ向きか。**
2. **5本のストップ定義すべてで同じ向きか。**
   (`1.5ATR(3-12%)` / `1.5ATR(clip無)` / `2.0ATR` / `固定5%` / `固定8%`)
3. **崖ではなく坂か。** — 線を1つ動かしただけで結果が急変するなら、その線は
   たまたま当たっているだけ。少しずつ動かして少しずつ変わるものだけ採る。
4. **その差は件数の割に大きいか(±2の幅を見る)。**
   期待Rのばらつき2つぶん(標準誤差×2)を出し、**比較相手の期待Rがその幅の中に
   入るなら「差がある」と書かない。**

**4番が要る理由**: 2番(5本のストップ定義で符号が揃う)を「偶然ではない証拠」として
使っていたが、これは間違い。5本は**同じ取引を5通りに見ているだけ**で、5回の独立した
観測ではない。件数が少ないと5本とも同じ向きに揃ってブレる。実際 178 では、n=154 の
群で5本全部が同じ向きだったのに、±2の幅を出したら比較相手が全部その中に入っていた
(0.333±0.319 など)。

**出し方**: `tools/audit_exit.py --part gate` が `stats()` の `seR` を使って
「その差は件数の割に大きいか」の表を出す。目安は **±2 ≒ 4/√n**。
n=100 なら ±0.4、n=1,000 なら ±0.13、n=10,000 なら ±0.04。
**期待Rの差が0.05前後の話をするなら数千件は要る。**

制定の経緯と 178 の実例は log.md の
「運用ルール: 検証結果を採用してよいかの判定 (2026-08-13 制定・恒久)」を参照。
