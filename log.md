...
1. **A-1 の比較に「現行の総合スコア順」が入っていない。** RS順・傾き順・ランダムの
   3つとしか比べていない。総合スコアには枯れ度が入っていて、枯れ度は25年で最も
   安定した指標なので、負ける可能性は消えていない。`tools/audit_exit.py --part rank`
   で実測してから既定値を確定させる。それまで設定で戻せるようにしてある。
2. 枯れ度と H3(52週安値倍率)は A-2 前の母集団(10.6万件)で測った数字。
   4.2万件で測り直す必要がある。
