...
1. ユーザーが自マシンで `python tools/fetch_long_history.py --start 2015-01-01`
   (sandbox からは Yahoo に到達できないため実行できない。プロキシが403を返す)。
2. `--verify` で調整基準の一致を確認。**ここが通るまで分析に進まない。**
3. `--survivorship-report` + JPX上場社数の照合でバイアス規模を確定。
4. その上で `tools/audit_sepa_thresholds.py` を `data/prices_long/` 対応にして
   134の診断を再実行し、**閾値ごとに符号がレジームを跨いで保たれるか**を見る。
